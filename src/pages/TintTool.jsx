...
const SLIDERS = [
  { key: 'hueShift', label: 'Hue shift', min: -45, max: 45, unit: '°', hint: 'Rotate hue toward the light end', snaps: [-30, -15, 0, 15, 30] },
  { key: 'satDecay', label: 'Saturation drift', min: 0, max: 40, unit: '%', hint: 'How much the ends desaturate', snaps: [0, 12, 24, 36] },
  { key: 'lMax', label: 'Lightest stop', min: 80, max: 99, unit: '%', hint: 'Lightness of the 50 swatch', snaps: [85, 90, 97] },
  { key: 'lMin', label: 'Darkest stop', min: 0, max: 25, unit: '%', hint: 'Lightness of the 950 swatch', snaps: [0, 4, 10, 15, 20] },
]
    ...
    <div className="sec">
      ...
      <div className="tt-grid">
        ...
        <div className="card tt-panel">
          ...
          {SLIDERS.map(({ key, label, min, max, unit, hint, snaps }) => (
            <div className="tt-slider-row" key={key}>
              ...
              <SnapSlider
                ...
                max={max}
                value={tuning[key]}
                defaultValue={DEFAULT_TUNING[key]}
                snaps={snaps}
                unit={unit}
