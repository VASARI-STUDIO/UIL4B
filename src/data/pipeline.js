...
export const NEXT_TODO = [
  { id: 'submissions-account-not-browser', title: 'Tie every user submission to the account, not the browser', priority: 'P1', effort: 'M', area: 'Community', status: 'todo', note: 'Founder report (2026-09-01): “user submissions should be tied to account not browser”. Gradients ALREADY are — GradientGenerator publishes via publishToQueue and GradientGallery merges the account copy back with listMySubmissions. Palettes and designs never got the same treatment: PaletteBuilder.submitToCommunity calls appendCommunitySubmission (localStorage only), so a palette submitted on a phone is invisible on a laptop AND never reaches a reviewer — the exact symptom already fixed for gradients. QUEUE_KINDS in utils/communityQueue.js already lists ‘palette’ and ‘design’, so the queue takes them today; only the call site is missing. Also browser-bound and in scope: Community.jsx reads vs-community-submissions alone, the vs-community-handle uniqueness check only sees handles set on this device (needs a Firestore rule or transaction), and vs-community-saves / vs-palette-likes / vs-gradient-likes are per-browser. Out of scope: vs-prompts, which is a private scratch list rather than a submission.' },
