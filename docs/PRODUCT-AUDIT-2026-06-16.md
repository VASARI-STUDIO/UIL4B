...
| Issue | Resolution |
|---|---|
| P0-1 Login `from` redirect | `Login.jsx` now reads `location.state.from` and returns the user there after email/Google sign-in |
| P0-2 Silent feedback failures | `Feedback.jsx` + `HelpCentre.jsx` now check `res.ok` and show an error instead of a false "thank you" |
| P0-3 `saveFeedback` wrong signature | `HelpCentre.jsx` now passes a single entry object (was 4 positional args) |
| P0-4 `scan-photo.js` uncapped | Added per-user daily limit mirroring `generate-prompt.js` (`toolId: 'scan-photo'`) |
| P1-1 Color Studio undo coverage | Undo now covers Apply-Brand, Apply-Design-System, Image-Extract, and swatch removal (was Reset only) |
| P1-2 Silent image-extract error | Surfaced via toast (and fixed the un-destructured `toast` prop that would have thrown) |
| P1-3 Named-colour search empty state | "No colours found" added |
| P1-4 FontMatcher stuck loading | `.catch` + error state with Retry |
| P1-6 PromptLibrary media drop | Warns the user when oversized media is dropped on submit |
| P1-7 AutoBuilder extract hang | try/catch/finally so a corrupt image surfaces an error and resets the button |
| P1-8 Emoji/FontGallery empty states | Added |
| UIBuilder clipboard | Unguarded `navigator.clipboard` calls now routed through the safe `onCopy` helper / wrapped in try/catch |
