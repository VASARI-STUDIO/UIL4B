...
| P1 | `https://uil4b.com/api/ai` — the OpenRouter path | **READ THIS BEFORE RUNNING IT: the diagnostic cannot tell you OpenRouter is available, and the previous version of this row asked you to confirm exactly that.** `?diag=1` reports key PRESENCE and length only — `openrouterKey: "set (73 chars)"` — which is the fact that was already confirmed on 2026-08-07. A wrong, revoked or rate-limited key is *also* "set (73 chars)". Ticking this row off the diagnostic would record a pass for a check that never happened, which is the same silent failure this item exists to catch. **The only thing that proves the route is the `provider` field on a real generation.** `POST /api/ai` with `{ "task": "generate-prompt", "description": "<anything>" }` returns `provider: "openrouter"` or `provider: "gemini"` in the response body. That field is the answer. **Procedure.** (1) Sign in to `https://uil4b.com` as the founder. (2) Open devtools → Network. (3) Go to the AI Image Prompt Generator and generate one prompt. (4) Open the `/api/ai` response and read `provider`. **How to read the result.** `provider: "openrouter"` → PASS, the primary provider is live; record the date here and close the item. `provider: "gemini"` → **FAIL, and the app looked healthy while you did it** — the generation succeeded, the UI showed nothing wrong, and the primary provider is dead. The reason is in the Vercel function logs for that invocation, on a line beginning `OpenRouter failed, will try Gemini fallback:` with the HTTP status; 401/403 means the key is wrong or revoked, 429 means rate-limited or out of credit, anything 5xx means an OpenRouter outage. HTTP 502 with "AI provider rejected the API key" → BOTH providers are down. HTTP 500 "AI is not configured" → neither key is set. **Do not skip the devtools step by watching whether it works.** It will work either way. That is the defect. Diagnostic access, if you want the key-presence report anyway: it requires a Firebase ID token from a verified admin email and answers **404** to a plain browser visit — `Authorization: Bearer <idToken>`, token copied from the signed-in app’s devtools. Break-glass for the one case admin auth cannot cover (a broken Firebase credential, the thing the diagnostic is most needed to report): set `DIAG_CODE` in Vercel to a value of your own and call `?diag=<that value>`. It has **no default** — unset means the break-glass does not exist. **Verified from outside on 2026-09-03, without credentials:** the route is live and correctly gated (`?diag=1` → 404, `POST` without a token → 401), and at least `GEMINI_API_KEY` is present in production. Nothing reachable without a founder session can say anything about the OpenRouter key. |
