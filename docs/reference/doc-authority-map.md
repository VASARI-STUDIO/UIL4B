...
| Ideas awaiting a founder verdict | `docs/PROPOSALS.md` — **local-only** | — no agent may claim a verdict not written on the verdict line. **Check it before asking:** five entries were answered on 2026-09-05 and re-asking one is the exact friction the founder complained about |
| Founder-only console / credential work | `docs/OWNER-ACTIONS.md` — **local-only** | — |
| How we take this to market | `docs/MARKETING.md` — **local-only** | — |
| Known-unfixed defects from the August sweep | `docs/qa/defect-register-2026-08.md` — **local-only** | — |
...
**“Local-only” is a seventh authority state, added 2026-09-16.** The repository
went public that day and the founder ruled that what had been written for an
internal reader stays on his machine: *"review whats in the public repo as this
is now public, anything sensative should be stored locally."* The file is still
canonical for its fact — the rows above are unchanged in that respect — it is
simply not in this checkout unless you are on his.

The four documents are not there for one reason, and saying so plainly matters
more here than anywhere, because this file is the one that decides what agents
trust. **`OWNER-ACTIONS.md` is a security call.** It is a current-state inventory
of what is *not yet secured*, written to be actioned console page by console
page, and it quotes the live rules diffs. **`PROPOSALS.md`, `MARKETING.md` and
`qa/defect-register-2026-08.md` are commercial caution** — an unreleased
roadmap, a market plan, a list of bugs we have not fixed. Nothing in those three
is exploitable, and calling them a security measure would devalue the word the
next time it is needed. `src/data/pipeline.js` and `src/data/moduleBoard.js` went
the same way on the same day, for the reason recorded in `.gitignore`.

**This creates an eighth failure mode and it is the map's business:** an agent
without the file, asked a question the file answers, inventing an answer. The
rule is the one already written for a verdict line — *no agent may claim what it
cannot point at*. Without the document, say the fact is local-only and ask.
Re-deriving `OWNER-ACTIONS.md` from the code is the specific temptation, and it
produces a *worse* copy of a security list in a *public* place.
...
| File | Verdict | Why |
|---|---|---|
| `PROPOSALS.md` | **LOCAL-ONLY 2026-09-16**, was UPDATE 2026-09-10 | Out of the public repository and kept on the founder's machine — commercial caution, not security: it is the unreleased roadmap and he would rather a competitor did not read it. Still canonical for its fact; still the only place a verdict is recorded. Founder verdict queue. Two entries were still `PENDING` after he had answered them: **P-006** (the hero — he retired the shape exploration on 2026-09-07 and approved the assembled headline on 2026-09-10) and **P-012** (the parked homepage PRs — he closed all three on 2026-09-10). Both resolved, both added to the Resolved table, and P-006's dead `/?hero=a\|b\|c` instruction removed. **P-023 was re-checked and is genuinely open**, so it stays. |
| `RELEASE-READINESS.md` | **RETIRED 2026-09-14** | Deleted on the founder's instruction. It was a second set of numbers for facts `OWNER-ACTIONS.md` already owned — the same blockers, renumbered — so the two could disagree, and had. Its release verdict is now the box at the top of `OWNER-ACTIONS.md`; its §4, the only part that was genuinely its own, is in `build-and-verify.md` under *What a green gate does NOT prove*. Do not recreate it: if a fact needs a home, it has one. |
| `OWNER-ACTIONS.md` | **LOCAL-ONLY 2026-09-16**, was RE-VERIFIED 2026-09-10 | **The security one, and the reason the whole convention exists.** It is a current-state inventory of what is not yet secured — the feedback queue open until the merged rules are published, Storage switched off, the Stripe webhook posting to the homepage, a Customer Portal never configured, an unrestricted Google key — with the console page and the exact step beside each, and it quotes the live `firestore.rules` diffs. That is a to-do list in a private repository and a worklist for a stranger in a public one. Do not reconstruct it here if you do not have it. Every item checked against the code. §2.1 closed (failure mode 6, above) and moved to §3; §1.1's "in flight" ffmpeg claim, §1.3's Java promise and §4.6's incomplete instruction corrected; six founder decisions surfaced by the #429–#439 audits added as §2.3–§2.6; the two sentence lists reconciled into one §2.2 with the file beside each slot. **2026-09-05 / 2026-09-06:** | Founder-gated actions, restructured on the founder’s instruction that our writing takes too long to parse. Every item carries Do / Time / Why / If you do nothing. §1 is what is stopped, §2 is the short list of what still needs him, §3 is the ten decisions he made on 2026-09-05 **restated so no agent asks twice — `CHANGELOG.md` remains their canonical home**, §4 onward is console work. `tests/unit/ai-provider-path.test.js` still pins the OpenRouter procedure. |
...
now split across `PRODUCT.md`, `docs/OWNER-ACTIONS.md`, `src/data/pipeline.js`
