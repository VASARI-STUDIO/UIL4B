...
//      #EFEEE9, #101012, #0B5ED7, #4A90FF. A palette change in global.css
...
test('the theme guide quotes this product’s own grounds and accents', () => {
  ...
  assert.equal(darkGround, '#101012')
...
test('the dark surface ramp spans what the guide says it spans', () => {
  ...
  assert.equal(levels[0], '#101012')
  assert.equal(levels[4], '#2A2B2F')
  assert.equal(levels.length, 5, 'the guide says five surface levels')
  assert.equal(shown(ratio(levels[0], levels[4])), '1.34:1')
  ...
  assert.equal(shown(Math.max(...steps)), '1.11:1',
    `the largest surface step is now ${shown(Math.max(...steps))}`)
  assert.match(themeProse, /1\.34:1/)
  assert.match(themeProse, /1\.04:1/)
  assert.match(themeProse, /1\.11:1/)
