...
//   -> 200, 1945 bytes, containing dylanjacob1100@gmail.com TWICE
//
...
//
// A grep for `dylanjacob1100@gmail.com` would go green the day somebody pastes
...
import { ADMIN_EMAILS as SERVER_ADMIN_EMAILS } from '../../api/_lib/plans.js'
import {
  ADMIN_EMAILS,
  isAdminEmail,
  ownerEmailDigest,
...
// The address itself, read from the SERVER-side allowlist — the one copy that
// is allowed to hold it, because api/_lib is never bundled. Reading it rather
// than retyping it means this file cannot be quietly pointed at a string the
// product no longer uses.
const FOUNDER_EMAIL = SERVER_ADMIN_EMAILS[0]
...
test('the address pattern finds a planted address, and the allowlist refuses it', () => {
  ...
  assert.equal(allowed(FOUNDER_EMAIL), false, 'the founder\'s own address is on the allowlist')
  fs.rmSync(dir, { recursive: true, force: true })
})

test('the raw search for the founder address finds it when it is there', () => {
  const dir = fixture({ 'planted.js': `const a = ${JSON.stringify(FOUNDER_EMAIL)}\n` })
  const { files } = scan(dir)
  assert.ok(files.some((f) => f.text.toLowerCase().includes(FOUNDER_EMAIL.toLowerCase())),
    'the substring search cannot see the address in a file that plainly contains it')
  fs.rmSync(dir, { recursive: true, force: true })
})
...
test('the founder address appears nowhere in dist/, in any file', { skip }, () => {
  const { files } = scan(DIST)
  const carrying = files
    .filter((f) => f.text.toLowerCase().includes(FOUNDER_EMAIL.toLowerCase()))
    .map((f) => `dist/${f.rel}`)
  assert.deepEqual(carrying, [],
    `${FOUNDER_EMAIL} is published to every visitor by:\n  ` + carrying.join('\n  '))
})
...
test('the SHIPPED chunk still recognises the founder', { skip }, async () => {
  const mod = await builtConstants()
  const answers = answersFor(mod, FOUNDER_EMAIL)
  assert.ok(answers.length >= 2, 'the built chunk exports nothing callable — this test is exercising nothing')
  assert.ok(answers.includes(true),
    'no function in the shipped constants chunk answers true for the founder\'s address: the /admin route guard, '
    + 'the nav item, the command palette and the Pro entitlement all go dark for him')
  // And it is a real check, not a function that says true to anybody.
...
test('the SHIPPED chunk still resolves the crown and the public handle', { skip }, async () => {
  const mod = await builtConstants()
  const byEmail = answersFor(mod, FOUNDER_EMAIL).find((v) => v && typeof v === 'object' && 'crown' in v)
  assert.ok(byEmail, 'the shipped chunk no longer maps the founder\'s address to an owner record — the crown is gone')
  assert.deepEqual({ ...byEmail }, OWNER_RECORD,
    'the shipped owner record is not the public identity it is supposed to be')

  // The other door into the same record: a public submission carries an
  // ownerId, not an address, and Community and PaletteBuilder render through it.
...
test('the public identity is intact in source, field by field', () => {
  const record = getOwnerHandle(FOUNDER_EMAIL)
  assert.ok(record, 'getOwnerHandle no longer resolves the founder')
  assert.deepEqual({ ...record }, OWNER_RECORD,
...
test('OWNER_HANDLES is keyed by a digest, not by an address', () => {
  ...
  assert.equal(key, ownerEmailDigest(FOUNDER_EMAIL),
    'the OWNER_HANDLES key is not the digest of the founder\'s address — the crown resolves for nobody')
})

test('the client check agrees with the server allowlist it mirrors', () => {
  assert.ok(SERVER_ADMIN_EMAILS.length > 0, 'the server allowlist is empty')
  for (const address of SERVER_ADMIN_EMAILS) {
    assert.equal(isAdminEmail(address), true,
      `api/_lib/plans.js grants ${address} admin and src/utils/constants.js does not — the two have drifted`)
    assert.equal(isAdminEmail(address.toUpperCase()), true, 'the client check stopped being case-insensitive')
  }
  for (const address of ['mallory@not-the-founder.example', `${FOUNDER_EMAIL}.uk`, '', null, undefined]) {
...
test('the founder-gated call site still gets a working admin check', () => {
  ...
  assert.equal(ADMIN_EMAILS.includes(FOUNDER_EMAIL.toLowerCase()), true,
    'the gated call site no longer recognises the founder: he loses Pro on every surface useSubscription gates')
...
test('the hand-written SHA-256 is a real SHA-256', () => {
  ...
  assert.match(ownerEmailDigest(FOUNDER_EMAIL), /^[0-9a-f]{64}$/)
})
