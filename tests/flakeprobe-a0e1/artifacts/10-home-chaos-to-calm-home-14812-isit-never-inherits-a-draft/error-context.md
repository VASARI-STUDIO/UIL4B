...
- Name: 10-home-chaos-to-calm.spec.js >> homepage: eleven tools, five ways of working >> 15 · a direct Icon Library visit never inherits a draft
- Location: tests\user-sim\10-home-chaos-to-calm.spec.js:1376:3

# Error details

```
Error: /create/icons: the route never got past its lazy-loading fallback in 20000ms — mounted=false fallback=false bodyChars=0 ownChars=0

  3 build asset request(s) FAILED in this browser context (404 /assets/IconLibrary-BVWszNSS.js, 404 /assets/SnapSlider-nPPKD6uN.js, 404 /assets/LibraryFilterGroup-Ci5X8Wl2.js). Files under /assets/ cannot 404 in a healthy run, so dist/ was almost certainly REBUILT while this suite was running — read the route below as a casualty of that, not as a defect. The global teardown fails the whole run on this.
```
