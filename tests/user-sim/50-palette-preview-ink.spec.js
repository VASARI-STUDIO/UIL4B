// The Palette Builder preview's two ink roles, measured as RENDERED text, on
// the ground each one is actually painted on, in both preview themes.
//
...
// Every leaf text node in a preview scene whose ink IS one of the two roles
// under test, identified by comparing the computed colour to the scene's own
// --pv-muted / --pv-text rather than by a class list — a class list goes stale
// the moment a scene is added, and six of these scenes were added at once.
const SWEEP = `(() => {
  ${HELPERS}
  const out = []
  for (const el of document.querySelectorAll('.plb-pv *')) {
    ...
    const hex = hexOf(ink)
    const role = hex === scene.getPropertyValue('--pv-muted').trim().toLowerCase() ? 'muted'
      : hex === scene.getPropertyValue('--pv-text').trim().toLowerCase() ? 'text'
        : null
    if (!role) continue
...
test.describe('Palette Builder preview ink', () => {
  test('muted and text clear their floor on every ground, in both preview themes', async ({ page }) => {
    ...
    for (const theme of ['Light', 'Dark']) {
      ...
      expect(roles.bg, `${theme}: --pv-bg is not a colour`).toMatch(/^#[0-9a-f]{6}$/)
      ...
      // 50 leaf nodes carry one of the two roles across the six visible scenes
      // (21 muted, 29 text) at the time of writing. The bar is set below that
      // rather than at it, so adding a scene does not fail the suite - but a
      // sweep that has stopped FINDING the ink still does.
      expect(samples.length, `${theme}: too few muted or text nodes were found`).toBeGreaterThan(40)

      for (const s of samples) {
        measured++
        if (s.ground === roles.surface) onSurface++
        if (s.ratio < s.floor) {
          failures.push(`  ${theme} .${s.cls.split(' ')[0]} ${s.role} ${s.ink} on ${s.ground}`
            + ` = ${s.ratio}:1 at ${s.size}px/${s.weight} (floor ${s.floor})`)
        }
      }
    }

    // Both themes produced DIFFERENT grounds, so the loop above ran twice for
    // real. PaletteBuilder's preview theme is a modal control, not the app's
