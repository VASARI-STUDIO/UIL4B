...
// /discover/palettes is here for the reason the list above is
...
const FILTER_TRAYS = [
  ...
  // ZERO collapsed triggers in the band: the icon row also
  // carries the weight segment, so from 641 to 980 its
  // groups step past the trigger form into the one Filters control — the
  // no-wrap S4 test above asserts that form.
  ['/create/icons', '.lbry-filters', '.lbry-filter', 1, 7, null, 0],
...
// chipRowDamage() lived here: it measured trays that were ALLOWED to wrap,
// which is the layout the no-wrap rule below retired. Its question is asked
// of the new layout by the test that follows.

// ── S4, REWRITTEN FOR THE NO-WRAP RULE ─────────────────
// Toolbars never wrap, on any width, desktop included. When controls don't
// fit, they collapse into a menu or sheet. So the old
// question — does every chip sit inside a tray that is allowed to wrap onto
// more lines? — is now the defect itself. What is asked instead, of every
// library surface at a phone and a desktop width:
//
//   · the toolbar's main row is ONE line: everything on it shares a top;
//   · every chip in an on-screen tray shares one line too, and sits inside
//     its tray — except the phone's quick row (`.lbry-toolbar-quick`), the one
//     container that is SUPPOSED to scroll sideways (one scrollable chip row
//     plus a filter sheet);
//   · the page itself never scrolls sideways;
//   · and whatever left the row is still reachable: a phone has a Filters
//     control whenever it has a group that is not on the quick row.
// Font Gallery is on the shared tool toolbar now; 100-tool-toolbar-one-row holds it.
const NO_WRAP_SURFACES = ['/discover/gradients', '/discover/palettes', '/create/emoji', '/create/icons', '/discover/prompts', '/discover/resources']
const NO_WRAP_WIDTHS = [320, 390, 1024, 1180, 1440]

test('S4 · no Library toolbar wraps, at any width, and nothing it hides is lost', async ({ browser }) => {
  budget(NO_WRAP_SURFACES.length * NO_WRAP_WIDTHS.length)
  const damage = []
  for (const path of NO_WRAP_SURFACES) {
    for (const w of NO_WRAP_WIDTHS) {
      const { ctx, page } = await open(browser, w, 900, path, '.lbry-toolbar', { touch: w < 800 })
      // Let the toolbar settle: it measures itself on fonts.ready and on every
      // resize, and steps down a stage at a time.
      await page.evaluate(() => document.fonts.ready.catch(() => {}))
      await page.waitForTimeout(250)
      const r = await page.evaluate(() => {
        const vis = (e) => e && e.offsetParent !== null
        const bar = [...document.querySelectorAll('.lbry-toolbar')].find(vis)
        if (!bar) return null
        const row = bar.querySelector(':scope > .lbry-toolbar-row')
        const tops = [...row.children].filter(vis).map((e) => Math.round(e.getBoundingClientRect().top + e.getBoundingClientRect().height / 2))
        const trays = [...bar.querySelectorAll('.lbry-filters')].filter(vis).map((t) => {
          const quick = !!t.closest('.lbry-toolbar-quick')
          const tb = t.getBoundingClientRect()
          const chips = [...t.querySelectorAll('.lbry-filter')].map((c) => c.getBoundingClientRect())
          return {
            name: t.getAttribute('aria-label'),
            quick,
            chips: chips.length,
            lines: new Set(chips.map((c) => Math.round(c.top))).size,
            outside: quick ? 0 : chips.filter((c) => c.right > tb.right + 0.5 || c.left < tb.left - 0.5).length,
          }
        })
        return {
          rowLines: new Set(tops).size,
          trays,
          filtersBtn: !!bar.querySelector('.lbry-filtersbtn'),
          stage: bar.dataset.stage,
          overflowX: document.documentElement.scrollWidth - document.documentElement.clientWidth,
        }
      })
      await ctx.close()
      if (!r) { damage.push(`${path} @${w}px: no toolbar rendered`); continue }
      if (r.rowLines !== 1) damage.push(`${path} @${w}px: the toolbar row wraps onto ${r.rowLines} lines (stage ${r.stage})`)
      for (const t of r.trays) {
        if (!t.chips) damage.push(`${path} @${w}px: tray “${t.name}” is empty`)
        if (t.lines > 1) damage.push(`${path} @${w}px: tray “${t.name}” wraps onto ${t.lines} lines`)
        if (t.outside) damage.push(`${path} @${w}px: ${t.outside} chip(s) outside tray “${t.name}”`)
      }
      if (r.overflowX > 1) damage.push(`${path} @${w}px: the page scrolls sideways by ${r.overflowX}px`)
      if (w < 641 && r.stage !== '2' && r.stage !== '3') damage.push(`${path} @${w}px: a phone toolbar at stage ${r.stage} — its groups should be behind Filters`)
    }
  }
  expect(damage, damage.join('\n')).toEqual([])
})
...
// Each role is one family row on the rebuilt screen (D:928-942) —
// name, token, its preset select and its ten steps. What S3 held (nothing of a
// role spills out of that role's row, and no row becomes a sideways scroller)
// is held for the new row.
test('S3 · every Semantic Colour role, preset and ramp is inside its own row', async ({ browser }) => {
  budget(STC_WIDTHS.length)
  const damage = []
  for (const w of STC_WIDTHS) {
    const { ctx, page } = await open(browser, w, 900, '/create/semantic-color', '.stc-fam', { touch: w < 800 })
    const r = await page.evaluate(() => {
      const rows = [...document.querySelectorAll('.stc-fam')]
      let outside = 0, total = 0, scrollers = 0
      for (const row of rows) {
        const rb = row.getBoundingClientRect()
        if (row.scrollWidth > row.clientWidth + 1) scrollers++
        for (const el of row.querySelectorAll('.stc-fam-preset, .stc-cell')) {
          total++
          const b = el.getBoundingClientRect()
          if (b.right > rb.right + 0.5 || b.left < rb.left - 0.5) outside++
        }
      }
      return { rows: rows.length, outside, total, scrollers, ramps: document.querySelectorAll('.stc-ramp').length, presets: document.querySelectorAll('.stc-fam-preset').length }
    })
    await ctx.close()
    expect(r.rows, `${w}px: no role rows rendered at all`).toBeGreaterThan(0)
    expect(r.rows, `${w}px: expected one row per ramp`).toBe(r.ramps)
    expect(r.presets, `${w}px: expected one preset control per role`).toBe(r.rows)
    if (r.outside) damage.push(`${w}px: ${r.outside} of ${r.total} controls outside their role's row`)
    if (r.scrollers) damage.push(`${w}px: ${r.scrollers} role row(s) are horizontal scrollers`)
  }
  expect(damage, damage.join('\n')).toEqual([])
})
...
test('S5 · the feedback FAB compacts on a short viewport and keeps its label on a roomy one', async ({ browser }) => {
  budget(FAB_SHORT.length + FAB_ROOMY.length + 1)
  const damage = []
  // On a touch screen the FAB is not drawn at all (feedback lives in the menus),
  // so the compact form is a mouse-and-short-window case.
  ...
  {
    const { ctx, page } = await open(browser, 844, 390, '/discover/prompts', '.lbry-toolbar')
    const f = await fabState(page)
    await ctx.close()
    if (f && (f.w || f.h)) damage.push(`844x390 touch: the FAB is drawn (${f.w}x${f.h}) where the menus carry feedback`)
  }
  expect(damage, damage.join('\n')).toEqual([])
})
...
// 481 and 600 joined when the stop list went to one column: between
...
test('N2 · the gradient stop hex input shows its whole value', async ({ browser }) => {
  ...
  for (const w of GGN_WIDTHS) {
    const { ctx, page } = await open(browser, w, 900, '/create/gradient', '.grd-hex', { touch: w < 800 })
    const r = await page.evaluate(() => {
      // The rebuilt screen shows each stop's hex as text in its
      // STOPS row and edits the selected one in a single field; all of them
      // must show the whole value in the mono face.
      const inputs = [...document.querySelectorAll('.grd-hex, .grd-stop-hex')]
      return {
        total: inputs.length,
        cut: inputs.filter((el) => el.scrollWidth > el.clientWidth + 1)
          .map((el) => `"${el.value || el.textContent}" needs ${el.scrollWidth}px, has ${el.clientWidth}px`),
...
test('the icon grid labels the pack only when the results actually mix packs', async ({ browser }) => {
  ...
  // A SINGLE PACK, CHOSEN, not "whatever the first page happens to hold". The
  // default grid is all packs interleaved 250 at a time, so it was single-pack
  // only while exactly one page of 120 had loaded — and the smaller masthead
  // puts the infinite-scroll sentinel on
  // the first screen at 1280, so a second page (Tabler) now loads on arrival
  // and the set genuinely mixes packs. Choosing Lucide is the state the rule
  // is about, and it cannot drift with the layout.
  await page.locator('select[aria-label="Icon pack"]').selectOption('lucide')
  await expect.poll(async () => (await read()).cells, { timeout: 20000 }).toBeGreaterThan(20)
  const single = await read()
  expect(single.cells, 'the Lucide grid should be populated').toBeGreaterThan(20)
  ...
  // It is a cross-pack collection, so the pack menu goes back to All first.
  await page.locator('select[aria-label="Icon pack"]').selectOption('all')
  // At this width the group is its one-line trigger (the toolbar never wraps),
  // so the chip lives in the menu it opens.
  const trig = page.locator('.ig-toolbar .lbry-filtertrig', { hasText: 'Group' })
