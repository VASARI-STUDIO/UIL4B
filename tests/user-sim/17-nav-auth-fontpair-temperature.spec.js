...
test.describe('nav auth opens over the page you are on', () => {
  test('Create a free account keeps the Palette Library underneath, and the X leaves you on it', async ({ page }) => {
    watch(page, 'browsing the palette library, signed out')
    // A short window, so the library's first screen leaves room to scroll.
    await page.setViewportSize({ width: 1280, height: 560 })
    await go(page, '/discover/palettes')

    // Signed out, sign-up lives in the header's Menu as "Create a free account".
...
test.describe('Font Pair · the sticky config panel is bounded by the viewport', () => {
  for (const viewport of [{ width: 1440, height: 900 }, { width: 1366, height: 768 }]) {
    test(`it fits at ${viewport.width}x${viewport.height}, and the hand-off is reachable`, async ({ page }) => {
      ...
      // Whatever no longer fits is scrolled, not clipped: the panel's last
      // control can be reached.
      const last = page.locator('.fpr-config #fpr-text')
      await last.scrollIntoViewIfNeeded()
      await expect(last).toBeInViewport()
      // The hand-off itself rides the sticky tool toolbar, so it is on screen.
      await expect(page.locator('[data-tool-toolbar]').getByRole('button', { name: 'Build a scale from this pair' })).toBeInViewport()
    })
  }
  ...
  test('it still fits when the panel content grows — a family with a third weight row', async ({ page }) => {
    ...
    // A long weight list or a longer catalogue notice grows the panel. Add
    // more height than the viewport has, directly, so the bound is tested
    // rather than a particular font's catalogue entry.
  ...
  test('below 901px it goes back to a plain stacked panel with no inner scroller', async ({ page }) => {
...
test.describe('Palette Builder · Temperature crosses zero continuously', () => {
  test('a slow drag across zero never leaps, and reaches values either side', async ({ page }) => {
    ...
    const readout = page.locator('.plb-adjust-field', { has: slider }).locator('.tl-slider-v').first()
